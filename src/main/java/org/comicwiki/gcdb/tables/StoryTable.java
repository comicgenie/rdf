...
import org.comicwiki.gcdb.fields.CharacterFieldParser;
import org.comicwiki.gcdb.fields.CreatorFieldParser;
...
public class StoryTable extends BaseTable<StoryTable.StoryRow> {
	...
	public static final class StoryRow extends TableRow {
		...
		@Join(field = "issue_id", table = IssueTable.class)
		public int issueId;
		...
		@Join(field = "type_id", table = StoryTypeTable.class)
		public int typeId;
	...
	@Override
	public void join(StoryRow storyRow) throws IOException {
	...
	public StoryRow process(Row row) throws IOException {
		...
		if (!row.isNullAt(Columns.ID)) {
			...
			add(storyRow.id, storyRow);
		}
		return storyRow;
	}

	@Override
	public void process(StoryRow row) {
		super.process(row);
